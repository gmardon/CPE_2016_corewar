...
** Last update Mon Dec  5 17:16:21 2016 Guillaume MARDON
*/
...
void	add_magic(char* buffer, int *index)
{
  ...
  char m2 = (COREWAR_EXEC_MAGIC >> 8) & 0xff;
  char m3 = COREWAR_EXEC_MAGIC & 0xff;
  buffer[*index] = 0x00;
  *index += 1;
  buffer[*index] = m1;
  *index += 1;
...
void	compile_header(char* buffer, int *index, char* name, char* comment)
{
  ...
  write_to_buffer(buffer, index, right_padding("name", 4, PROG_NAME_LENGTH),
		  PROG_NAME_LENGTH);
}
